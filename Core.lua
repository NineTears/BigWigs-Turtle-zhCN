...
BigWigs.cmdtable = { type = "group", handler = BigWigs, args = {
	...
	["editlayout"] = {
		...
		name = L["Edit Layout"],
		order = 4,
		desc = L["Edit frame layout and alert sizes"],
		func = function()
			BigWigs:EditLayout()
		end,
		disabled = function()
...
function BigWigs:CheckForEngage(module)
	if module and module:IsBossModule() and not module.engaged then
		...
		if inCombat then
			module:DebugMessage("扫描结果为真，正在交战 ["..module:ToString().."].")
...
function BigWigs:CheckForWipe(module)
	if module and module:IsBossModule() then
		...
		if not RaidMemberAlive() or not RaidMemberInCombat() then
			module:DebugMessage("检测到模块清除 [" .. module:ToString() .. "].")
...
function BigWigs:AceEvent_FullyInitialized()
	if (GetNumRaidMembers() > 0) or ((GetZoneText() == BZ["The Black Morass"]) or (GetZoneText() == BZ["Dire Maul"]) or (GetZoneText() == BZ["Stratholme"])) or not self.loading then
...
function BigWigs:ModuleDeclaration(bossName, zoneName)
	--translatedName = AceLibrary("Babble-Boss-2.2")[bossName]-- balakethelock -
	translatedName = BB:HasTranslation(bossName) and BB[bossName] or bossName-- balakethelock +
	...
	for i, value in ipairs(raidZones) do
		if value == zoneName then
			--module.zonename = AceLibrary("Babble-Zone-2.2")[zoneName]-- balakethelock -
			module.zonename = BZ:HasTranslation(zoneName) and BZ[zoneName] or zoneName-- balakethelock +
			isOutdoorraid = false
			break
		end
	end
	if isOutdoorraid then
		module.zonename = {
			...
			--AceLibrary("Babble-Zone-2.2")[zoneName]-- balakethelock -
			BZ:HasTranslation(zoneName) and BZ[zoneName] or zoneName-- balakethelock +
		}
	end
