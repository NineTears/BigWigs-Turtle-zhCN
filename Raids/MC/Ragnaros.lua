...
local bbmajordomoexecutus = AceLibrary("Babble-Boss-2.2")["Majordomo Executus"]
...
L:RegisterTranslations("enUS", function() return {
    ...
    start_cmd = "start",
    start_name = "开始",
    start_desc = "创建一个计时条用于估计战斗开始的时间。",

    emerge_cmd = "emerge",
    emerge_name = "浮现警报",
    emerge_desc = "拉格纳罗斯浮现时发出警报",

    adds_cmd = "adds",
    adds_name = "烈焰之子死亡",
    adds_desc = "烈焰之子死亡时发出警报",

    submerge_cmd = "submerge",
    submerge_name = "下潜警报",
    submerge_desc = "拉格纳罗斯下潜时发出警报",

    aoeknock_cmd = "aoeknock",
    aoeknock_name = "击退警告",
    aoeknock_desc = "拉格纳罗斯的狂怒之击击退效果警告",
    
    trigger_knockback = "^TASTE",
    bar_knockback = "范围击退",
    msg_knockback = "击退！",
    msg_knockbackSoon = "即将击退！",
    
    trigger_submerge = "^COME FORTH,",
    trigger_submerge2 = "^YOU CANNOT DEFEAT THE LIVING FLAME,",
    bar_emerge = "拉格纳罗斯浮现",
    msg_submerge = "拉格纳罗斯已下潜。烈焰之子即将到来！",
    ...
    msg_emergeSoon = "拉格纳罗斯10秒后浮现！",
    msg_emerge = "拉格纳罗斯已浮现，3分钟后下潜！",
...
function module:EmergeCheck()
	if UnitExists("target") and UnitName("target") == module.translatedName and UnitExists("targettarget") and UnitName("targettarget") ~= bbmajordomoexecutus then
	...
	for i = 1, num do
		...
		if UnitExists(raidUnit) and UnitName(raidUnit) == module.translatedName and UnitExists(raidUnit .. "target") and UnitName(raidUnit .. "target") ~= bbmajordomoexecutus then
			self:Sync(syncName.emerge)
			return
		end
	end
