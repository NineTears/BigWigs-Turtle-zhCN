...
local coloredNames = setmetatable({}, { __index = function(self, unit)
...
BigWigsProximity.consoleOptions = {
	...
	args = {
		...
		sound = {
			...
			order = 100,
			get = function()
				return BigWigsProximity.db.profile.sound
			end,
			set = function(v)
				BigWigsProximity.db.profile.sound = v
			end,
		},
		disabled = {
			type = "toggle",
			...
			order = 101,
			get = function()
				return BigWigsProximity.db.profile.disabled
			end,
			set = function(v)
...
function BigWigsProximity:OpenProximity(module)
	...
	if module and not module.db.profile.proximity then
		return
	end
	...
	for k in pairs(tooClose) do
		tooClose[k] = nil
	end
	for k in pairs(tooClose2) do
		tooClose2[k] = nil
	end
...
function BigWigsProximity:UpdateProximity()
	...
	end -- 1=10(?) yards; 2=11.11 yards; 3=9.9 yards; 4=28 yards
	...
	if tablelength(tooClose) == 0 then
		anchor:SetBackdropBorderColor(0.0, 1.0, 0.0)
		--- green
		anchor.text:SetText(L["|cff777777Nobody|r"])
		anchor.text2:SetText("")
	else
		anchor:SetBackdropBorderColor(1.0, 0.0, 0.0)
		--- red
...
function BigWigsProximity:UpdateDebuff()
	...
	for i = 1, num do
		...
		if tablelength(hasDebuff) > 4 then
			break
		end
	end
